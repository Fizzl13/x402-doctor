...
async function preflight(targetUrl, { safeFetch, method, maxUsd, network, rpcUrl = process.env.SOLANA_RPC_URL || DEFAULT_SOLANA_RPC, bazaarIndex } = {}) {
  const url = new URL(targetUrl);
  const checks = [];
  const reasons = [];
  ...
  const [advertised, bazaar] = await Promise.all([
