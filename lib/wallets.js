...
function walletCompatibility(accepts, checks) {
  ...
  for (const wallet of WALLETS) {
    ...
    for (const o of options) {
      ...
      else if (!yes.includes(label)) yes.push(label);
    }
    if (yes.length || no.length) wallets.push({ wallet: wallet.name, agent: Boolean(wallet.agent), yes, no });
  }
  return wallets;
}
