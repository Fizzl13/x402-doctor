...
async function checkEvmPayTo(accepts, checks, { evmRpcUrls } = {}) {
  ...
  for (const [index, option] of (accepts || []).entries()) {
    ...
    if (typeof code === 'string' && /^0x0*$/.test(code)) {
      addCheck(checks, 'evm-payto-eoa', 'info', `payTo ${option.payTo} on ${net.name} is a regular wallet (EOA). MetaMask's security check (Blockaid) may flag the payment signature as "a deceptive request" because it doesn't know this wallet, which can scare off browser buyers. Agents are not affected.`, {
        group,
        option: index,
...
async function diagnose(targetUrl, { safeFetch, method, rpcUrl = process.env.SOLANA_RPC_URL || DEFAULT_SOLANA_RPC, evmRpcUrls } = {}) {
  ...
  return { url: url.href, method: probe?.method || null, overall, checks, wallets, challenge };
}
