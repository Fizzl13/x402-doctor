...
// USDC on Base and/or Solana (whichever payout wallet is configured), verified
// and settled by PayAI. The x402 middleware only settles after a successful
// (2xx) response, so a diagnosis that errors out is never charged.
...
function createPaidApi({ safeFetch, env = process.env } = {}) {
  ...
  const facilitator = new HTTPFacilitatorClient({ url: env.FACILITATOR_URL || DEFAULT_FACILITATOR });
  ...
  const discovery = declareDiscoveryExtension({
    ...
    inputSchema: {
      properties: {
        url: { type: 'string', format: 'uri', description: 'The x402 endpoint to diagnose (public http(s) URL).' },
  ...
  router.paymentInfo = { route: ROUTE, price, networks: accepts.map((a) => a.network) };
  return router;
}
