...
function paymentConfig(env) {
  ...
  return { price, preflightPrice, fixPrice, accepts: acceptsFor(price), preflightAccepts: acceptsFor(preflightPrice), fixAccepts: acceptsFor(fixPrice) };
}
...
function validate(req, res, next) {
  ...
  if (req.query.stack !== undefined && !STACKS[req.query.stack]) {
    return res.status(400).json({ error: `stack must be one of ${Object.keys(STACKS).join(', ')}` });
  }
...
function createPaidApi({ safeFetch, env = process.env, bazaarIndex = createBazaarIndex(), trustIndex = createTrustIndex() } = {}) {
  const router = express.Router();
  const { price, preflightPrice, fixPrice, accepts, preflightAccepts, fixAccepts } = paymentConfig(env);
  ...
  const fixExample = 'https://x402-doctor.onrender.com/demo/broken';
  ...
  const runPreflight = createPreflight({ safeFetch, bazaarIndex, trustIndex });
  ...
  router.use(createMcpRouter({
    resourceServer,
    config: { price, preflightPrice, fixPrice, accepts, preflightAccepts, fixAccepts },
    diagnose: (url, method) => diagnoseLib.diagnose(String(url), { safeFetch, method }),
    ...
    paymentMiddleware(
      {
        ...
        [`GET ${FIX_ROUTE}`]: {
          accepts: fixAccepts,
          description:
            'The fix for your x402 endpoint: diagnoses it, then returns per problem the concrete change as code for your stack (Express, Next.js, Hono or raw HTTP), filled in with your own payTo, amount, network and route.',
          mimeType: 'application/json',
          serviceName: 'x402 Doctor',
  ...
  router.get(FIX_ROUTE, async (req, res) => {
    ...
    if (url === undefined) return res.status(400).json({ error: 'url is required, e.g. ?url=https://api.example.com/paid' });
    try {
      const report = await diagnoseLib.diagnose(String(url), { safeFetch, method });
...
module.exports = { createPaidApi, onlyNetworks, ROUTE, PREFLIGHT_ROUTE, FIX_ROUTE, REPORT_SCHEMA, FIX_SCHEMA, paymentConfig };
