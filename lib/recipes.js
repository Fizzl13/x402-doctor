...
// an empty {} body; some clients read accepts[] from the body, so mirror it there.
      ...
      if (statuses.some((s) => s >= 200 && s < 300)) {
        return {
          ...
          code: [
            ...
            node('Free calls only when asked for', `// Everyone else gets the 402 challenge first.
const wantsFree = req.get("x-free-tier") === "1";
        ...
        paths: {
          [ctx.path]: {
            [ctx.method.toLowerCase()]: {
              ...
              'x-payment-info': { price: { mode: 'fixed', currency: 'USD', amount: ctx.accepts[0]?.amount && /^\d+$/.test(ctx.accepts[0].amount) ? (Number(ctx.accepts[0].amount) / 1e6).toString() : '0.01' }, protocols: ['x402'], networks: [...new Set(ctx.accepts.map((a) => a.network))], asset: 'USDC' },
...
function buildFixes(report, { stack: override } = {}) {
  ...
  const ctx = {
    ...
    origin: url.origin,
    path: url.pathname,
    method: report.method || (report.probes || []).find((p) => p.status === 402)?.method || 'GET',
