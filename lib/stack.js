...
function detectStack(report, override) {
  ...
  if (/next\.js/i.test(powered) || h['x-nextjs-cache'] || h['x-matched-path']) return pick('next', `x-powered-by: ${powered || 'Next.js headers'}`);
  ...
  if (h['x-vercel-id']) return pick('node', 'x-vercel-id (Vercel)');
